...
import { X, Eye, EyeOff, Building2, Lock, Mail, User, Briefcase, Loader2, CheckCircle } from "lucide-react";
  ...
  if (success) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
        <div className="w-full max-w-md bg-[#141a14] border border-[#2a3a2a] rounded-2xl shadow-2xl p-8 text-center">
          <div className="w-16 h-16 bg-[#1e3a1e] rounded-full flex items-center justify-center mx-auto mb-4">
            <CheckCircle size={32} className="text-[#4ade80]" />
          </div>
          <h2 className="text-xl font-bold text-white mb-2">Account Created!</h2>
          <p className="text-gray-400 text-sm">Welcome to Archaleon, {name}. You're now signed in.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="relative w-full max-w-md bg-[#141a14] border border-[#2a3a2a] rounded-2xl shadow-2xl p-8">
        ...
        <p className="text-gray-400 text-sm mb-6">
          {mode === "login" ? "Sign in to access the platform" : "Sign up to list businesses and connect with investors"}
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            {mode === "register" && password.length > 0 && password.length < 6 && (
              <p className="text-amber-400 text-xs mt-1">Password needs {6 - password.length} more character{6 - password.length > 1 ? "s" : ""}</p>
            )}
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-[#2d5a27] hover:bg-[#3a7232] text-white font-semibold py-3 rounded-lg transition-colors mt-2 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            {loading ? (
        ...
        <div className="mt-6 text-center text-sm text-gray-500">
          {mode === "login" ? (
            <>Don't have an account?{" "}
              <button onClick={() => { setMode("register"); setError(""); }} className="text-[#4ade80] hover:underline">
                Sign up
              </button>
            </>
          ) : (
            <>Already have an account?{" "}
              <button onClick={() => { setMode("login"); setError(""); }} className="text-[#4ade80] hover:underline">
                Sign in
              </button>
            </>
          )}
